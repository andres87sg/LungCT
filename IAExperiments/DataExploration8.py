...
def getfeaturematrix(df):
    ...
    class_two=dfclass_two.iloc[:,1:7].values
    class_three=dfclass_three.iloc[:,1:7].values
...
def getfeaturematrixnormalization(featurematrix):

    scaler = preprocessing.StandardScaler().fit(featurematrix)
    featurematrixscal=scaler.transform(featurematrix)

    return  featurematrixscal
...
df_train = getfeaturesdataframe(path,pathmask)

featurematrix_train,labels_train = getfeaturematrix(df_train)
featurematrixnorm_train = getfeaturematrixnormalization(featurematrix_train)

df_val = getfeaturesdataframe(path_val,pathmask_val)
...
df_test = getfeaturesdataframe(path_test,pathmask_test)

featurematrix_test,labels_test = getfeaturematrix(df_test)
featurematrixnorm_test = getfeaturematrixnormalization(featurematrix_test)
...
history = model.fit(featurematrixnorm_train,labels_train,
          validation_data=(featurematrixnorm_val, labels_val),
          epochs=100,verbose=0,callbacks=[es,mc,lr])
...
predict = model.predict(featurematrixnorm_test,verbose=0)


predicted_label=np.zeros(len(predict))

for ind in range(len(predict)):
    current_label=predict[ind,:].argmax()
    predicted_label[ind]=current_label
...
#predict_eval = model.evaluate(featurematrixnorm_test,verbose=0)
